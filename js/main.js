$(document).ready(() => {
    $("#hoTen").blur(() => {
        ...
        if (!result) {
            $('.option-hoTen').text("Họ tên không được bỏ trống, họ tên phải từ 2 từ trở lên, phải viết hoa chữ cái đầu.");
        } else {
    ...
    $("#email").blur(() => {
        ...
        var result = regex.test(hoTenVal);
        if (!result) {
            $('.option-email').text("Email không được bỏ trống, Email  phải có @");
        } else {
